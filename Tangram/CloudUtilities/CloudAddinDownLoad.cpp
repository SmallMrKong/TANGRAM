...
namespace CloudUtilities
{
	...
	BOOL CDownLoadObj::OnDownLoadSuccess(int id)
	{
		...
		{
			CloudUtilities::CComponentInstaller m_Installer;
	...
	BOOL CDownLoadObj::DownLoadFile(CString strURL, CString strTarget)
	{
		...
		auto t = create_task([strURL, strTarget,this] 
		{
			...
			try
			{
				...
				{
					::PostMessage(theApp.m_pMainWnd->m_hWnd, WM_DOWNLOAD_MSG, (WPARAM)this, (LPARAM)false);
				}
				else
				{
					OnDownLoadSuccess(0);
