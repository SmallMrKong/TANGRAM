...
#include "XZip.h"
#include "XUnzip.h"
#include "CloudAddinComponentInstaller.h"
...
namespace CloudUtilities
{
						...
						if (zr == ZR_OK)
						{
							...
							for (int i = 0; i < numitems; i++)
							{
								...
								else if (strName.CompareNoCase(_T("Templates")) == 0)
								{
									...
									if (hModule)
									{
										GetModuleFileName(hModule, theApp.m_szBuffer, MAX_PATH * sizeof(TCHAR));
										m_strModulePath = CString(theApp.m_szBuffer);
	...
	BOOL CComponentInstaller::Zip(LPCTSTR lpszZipArchive, LPCTSTR lpszSrcFile)
	{
		...
		HZIP hz = CloudUtilities::CreateZip((void *)lpszZipArchive, 0, ZIP_FILENAME);

		if (hz)
		{
			ZRESULT zr = CloudUtilities::ZipAdd(hz, cp, (void *)lpszSrcFile, 0, ZIP_FILENAME);
