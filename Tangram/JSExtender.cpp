...
CJSExtender::~CJSExtender(void)
{
	...
	for (auto it : m_mapCloudJSObject)
		delete it.second;
	m_mapCloudJSObject.erase(m_mapCloudJSObject.begin(), m_mapCloudJSObject.end());
...
{
	for (auto it = m_mapCloudJSObject.begin(); it != m_mapCloudJSObject.end(); it++)
	{
		if (pUnk == it->second->m_pDisp)
		{
			delete it->second;
			m_mapCloudJSObject.erase(it);
			return S_OK;
		}
	}
