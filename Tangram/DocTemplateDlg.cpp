...
BOOL CDocTemplateDlg::OnInitDialog()
{
	...
	m_strDocTemplatePath = theApp.m_strAppPath + m_strDir;
	// build a string with wildcards
	CString strWildcard = theApp.m_strAppPath + m_strDir + _T("*.*");
	...
	while (bWorking)
	{
		...
		int nPos = strFileName2.Find(_T(".xtml"));
		if(nPos!=-1)
		{
	...
	else
	{
		while (pos)
		{
			...
			if(strCurTabText==_T("Normal"))
				m_strTemplateDoc = m_strDocTemplatePath + strItemText + _T(".xtml");
			else
			{
...
{
	//g_pTangramImpl->SetTangramStringX(TANGRAMSTR_CURDOCTEMPLATEID,_T(""));
