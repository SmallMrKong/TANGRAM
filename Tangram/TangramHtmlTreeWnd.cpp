...
//=============================================================================
	...
	ON_MESSAGE(WM_TANGRAMDESIGNERCMD, OnDesignNode)
	ON_MESSAGE(WM_TANGRAMUPDATENODE,OnUpdateXMLNode)
	ON_MESSAGE(WM_INSERTTREENODE,OnInsertXMLNode)	
	ON_MESSAGE(WM_TANGRAMGETTREEINFO,OnGetTangramXmlParse)
	...
	m_dwDragOps(XHTMLTREE_DO_DEFAULT)
{
	...
	m_pObj->AddRef();
	m_pObj->m_pTangramHtmlTreeWnd = this;
...
void CTangramHtmlTreeWnd::UpdateData(CString strData)
{
	...
	if (_pHostXmlParse->LoadXml(strXml))
	{
		...
		if(theApp.m_pHostDesignUINode)
			theApp.m_pHostDesignUINode->m_pDocXmlParseNode = m_pHostXmlParse;
...
int CTangramHtmlTreeWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	if (m_pHostWnd)
	{
		theApp.m_pWndNode = m_pHostWnd->m_pWndNode;
		theApp.m_pWndFrame = m_pHostWnd->m_pWndNode->m_pFrame;
	}
	return MA_ACTIVATE;
...
//=============================================================================
{
	...
	if (pData)
	{
		...
		CTangramTreeNode* pNode			= new CComObject<CTangramTreeNode>;
		pNode->m_pXHTMLTREEDATA			= pXTCD;
		pXTCD->m_pTangramTreeNode		= pNode;//new CComObject<CTangramTreeNode>;
		pXTCD->m_pTangramTreeNode->AddRef();
...
//=============================================================================
void CTangramHtmlTreeWnd::Init(CWndNode* pNode)
//=============================================================================
{
	if (pNode)
	{
		CWndNode* pRootNode = pNode->m_pRootObj;
		if (pRootNode == NULL)
			pRootNode = pNode;
		CComBSTR bstrCallBackNode(L"");
		pNode->get_Attribute(CComBSTR(L"CallBackNode"), &bstrCallBackNode);
		...
		if (strCallBackNode != _T(""))
		{
			...
			if (nPos == -1)
			{
				...
				pRootNode->GetNodes(CComBSTR(strCallBackNode), &pNode, &pTangramNodeCollection, &nCount);
				if (pNode)
				{
					CComPtr<IDispatch> pdisp;
					pNode->get_Extender(&pdisp);
					if (pdisp)
					{
						CComQIPtr<ITangramTreeViewCallBack> pITangramTreeViewCallBack(pdisp);
						if (pITangramTreeViewCallBack)
						{
							m_pObj->put_TangramTreeViewCallBack(bstrCallBackNode, pITangramTreeViewCallBack.p);
							pITangramTreeViewCallBack->OnInitTreeView(m_pObj, CComBSTR(L"MainIMHistory"));
						}
					}
				}
			}
			while (nPos != -1)
			{
				...
				if (strID != _T(""))
				{
					IWndNode* pNode = NULL;
					CComPtr<IWndNodeCollection> pTangramNodeCollection;
					pRootNode->GetNodes(CComBSTR(strID), &pNode, &pTangramNodeCollection, &nCount);
					if (pNode)
					{
						CComPtr<IDispatch> pdisp;
						pNode->get_Extender(&pdisp);
						if (pdisp)
						{
							...
							if (pITangramTreeViewCallBack)
							{
								m_pObj->put_TangramTreeViewCallBack(strID.AllocSysString(), pITangramTreeViewCallBack.p);
							}
						}
					}
				}
		...
		if (pXTCD->m_strTangramXML != _T("")/*&&m_LasthItem!=hItem*/)
		{
			...
			if (m_Parse.LoadXml(pXTCD->m_strTangramXML))
			{
				...
				//			LONGLONG h = 0;
				//			this->m_pHostWnd->m_pWndNode->get_BindWnd(CComBSTR(strKey), &h);
				//			if (h)
				//			{
				//				CComPtr<IWndNode> pNode2;
				//				CString strXml = pParse->GetChild(strKey)->xml();
				//				CComPtr<IWndNode> _pNode;
				//				theApp.m_pTangramManager->NavigateXTML(h, strXml.AllocSysString(), &_pNode);
				//				//pNode->Navigate(CComBSTR(strKey),CComBSTR(strXml),&pNode2);
				//			}
				//			nPos = strIDs.Find(_T(","));
				//			if (nPos == -1)
				//				break;
				...
				//		CComPtr<IWndNodeCollection> pNodeCollection;
				//		pRootNode->GetNodes(CComBSTR(strKey), &pNode, &pNodeCollection, &nCount);
				//		if (pNode)
				//		{
				//			CComPtr<IWndNode> pNode2;
				//			CString strXml = pParse->GetChild(strKey)->xml();
	...
	if(hItem)
	{
		...
		if (pXTCD->m_bEnabled)
		{
			...
			if (theApp.m_pDocDOMTree == this)
			{
				...
				{
					CWndNode* pRootNode = theApp.m_pDesignWindowNode->m_pRootObj;
					IWndNode* _pNode = NULL;
					long nCount = 0;
					CComPtr<IWndNodeCollection> pCol;
					pRootNode->GetNodes(strNodeName.AllocSysString(), &_pNode, &pCol, &nCount);
					if (_pNode)
					{
						if (theApp.m_pCloudAddinCLRProxy)
							theApp.m_pCloudAddinCLRProxy->SelectNode(_pNode);
					}
				}
			}
			...
			if (m_pHostWnd)
			{
				m_pHostWnd->m_pWndNode->get_RootNode(&pRootNode);
			}
			if (pXTCD->m_strTangramXML != _T("")/*&&m_LasthItem!=hItem*/)
			{
				CTangramXmlParse m_Parse;
				if (m_Parse.LoadXml(pXTCD->m_strTangramXML))
				{
					...
					if (strBindWnds != _T(""))
					{
						...
						if (pParse)
						{
							...
							while (nPos != -1)
							{
								...
								//{
								//	CComPtr<IWndNode> pNode2;
								//	CTangramXmlParse* pParse2 = pParse->GetChild(strKey);
								//	if (pParse2)
								//	{
								//		CString strXml = pParse2->xml();
								//		if (pParse2->GetChild(_T("Window")))
								//		{
								//			CComPtr<IWndNode> _pNode;
								//			theApp.m_pTangramManager->NavigateXTML(h, strXml.AllocSysString(), &_pNode);
								//			CTangramXmlParse* pParse3 = pParse2->GetChild(_T("Click"));
								//			if (pParse3)
								//			{
								//				_pNode->get_Handle(&h);
								//				CString _strXml = pParse3->xml();
								//				bool bIsCLRExt = pParse3->attrBool(_T("CLRExt"), false);
								//				if (bIsCLRExt)
								//				{
								//					CString strVer = pParse3->attr(_T("CLRVer"), _T("v4.0.30319"));
								//					CLRRuntimeHostCollection::iterator it = theApp.m_CLRRuntimeHostCollection.find(strVer);
								//					if (it != theApp.m_CLRRuntimeHostCollection.end())
								//					{
								//						CComVariant var;
								//						it->second->m_pCloudAddinCLRProxy->TangramAction(CComBSTR(_strXml), _pNode, &var);
								//					}
								//				}
								//				::SendMessage((HWND)h, WM_TANGRAMMSG, (WPARAM)_strXml.GetBuffer(), 1024);
								//			}
								//		}
								//	}
								//}
					...
					//						{
					//							CComVariant var;
					//							it->second->m_pCloudAddinCLRProxy->TangramAction(CComBSTR(_strXml), pNode, &var);
					//						}
					//					}
					//					::SendMessage((HWND)h, WM_TANGRAMMSG, (WPARAM)_strXml.GetBuffer(), 1024);
					//				}
...
//=============================================================================
{
	...
	//if(m_pWebBrowser2)
	//{
	//	theApp.m_pTangramManager->PopupMenu(0,0,CComBSTR(m_pHostWnd->m_strXmlMenuScript),m_pWebBrowser2,(LONGLONG)m_hWnd);
	//}
	//else
	...
	//{
	//	CTangramXHtmlTreeNode *pXTCD = GetItemDataStruct(hItem);
	//	if (pXTCD&&theApp.m_hMainWnd&&pXTCD->m_strTangramMenuXML!=_T(""))		// will not be NULL if hItem is valid
	//	{
	//		//
	//		if(m_pWebBrowser2)
	//		{
	//			theApp.m_pCurWebBrowser = m_pWebBrowser2;
	//			theApp.m_pCurWebBrowser = NULL;
	//		}
	//		else
	//		{
	//			::SendMessage(theApp.m_hMainWnd,WM_MENU,(WPARAM)m_hWnd,(LPARAM)pXTCD->m_strTangramMenuXML.GetBuffer());
	//			pXTCD->m_strTangramMenuXML.ReleaseBuffer();
	//		}
	//	}
		...
		else 
		{
			...
			if (bHasChild == false)
			{
				if (pXTCD->m_strTangramXML != _T("")/*&&m_LasthItem!=hItem*/)
				{
					...
					//							theApp.m_pTangramManager->NavigateXTML(h, strXml.AllocSysString(), &_pNode);
					//						}
					//						CTangramXmlParse* pParse3 = pParse2->GetChild(_T("DblClick"));
					//						if (pParse3)
					//						{
					//							this->m_pHostWnd->m_pWndNode->get_Handle(&h);
					//							CString _strXml = pParse3->xml();
					//							bool bIsCLRExt = pParse3->attrBool(_T("CLRExt"), false);
					//							if (bIsCLRExt)
					//							{
					...
					//								{
					//									CComVariant var;
					//									it->second->m_pCloudAddinCLRProxy->TangramAction(CComBSTR(_strXml), m_pHostWnd->m_pWndNode, &var);
					//								}
					//							}
					//						}
					//					}
...
//=============================================================================
{
	...
	if(pXTCD->m_strTangramXML!=_T("")/*&&m_LasthItem!=hItem*/)
	{
		...
		if(m_Parse.LoadXml(pXTCD->m_strTangramXML))
		{
			...
			if(strProperty!=_T(""))
			{
				if(m_pObj)
				{
					...
					map<CString,ITangramTreeViewCallBack*>::iterator it = m_pObj->m_mapTangramTreeViewCallBack.end();
					for(it = m_pObj->m_mapTangramTreeViewCallBack.begin(); it!=m_pObj->m_mapTangramTreeViewCallBack.end();it++)
					{
			...
			else
			{
				...
				//						}
				//					}
				//					::SendMessage((HWND)h, WM_TANGRAMMSG, (WPARAM)pParse3->xml().GetBuffer(), 1024);
				//				}
				//			}
				//		}
				//		nPos = strIDs.Find(_T(","));
		...
		if(m_pWndFrame==NULL)
		{
			...
			////theApp.m_pTangramManager->NavigateXTML((LONGLONG)m_hWnd,CComBSTR(pXTCD->m_strTangramXML),&pNode);
			//m_bAttached = true;
			//if(m_pWndFrame==NULL)
			//{
			//	HRESULT hr = pNode->get_Frame(&m_pWndFrame);
			//	if(hr==S_OK)
			//		m_pWndFrame->AddRef();
			//}
		}
		if(m_hPreviousItem != hItem)
		{
			...
			m_bAttached = true;
			IWndNode* pNode = NULL;
			//theApp.m_pTangramManager->NavigateXTML((long)m_hWnd,CComBSTR(pXTCD->m_strTangramXML),&pNode);
		}
	}
	else if(pXTCD->m_strTangramXML==_T("")&&m_bAttached)
	{
		if(m_pWndFrame&&m_hPreviousItem)
		{
			//m_pWndFrame->Deatch();
			m_bAttached = false;
		}
		...
		if(m_Parse.LoadXml(lpszXML))
		{
			...
			else
			{
				...
				if (pParse)
				{
					...
					//	//m_pTangramRestObj->put_Header(CComBSTR(L"FileId"), CComBSTR(_strID));
					//	CComQIPtr<IRestNotify> pCloudAddinRestNotify(this->m_pHostWnd->m_pWndNode);
					//	m_pTangramRestObj->put_NotifyHandle((LONGLONG)m_hWnd);
					//	m_pTangramRestObj->RestData(1, strURL.AllocSysString(), L"", L"", 0);
					//}
				}
				//else if (theApp.m_pTangramManager)
				//{
				//	if(m_strMainKey==_T(""))
				//	{
...
LRESULT CTangramHtmlTreeWnd::OnGetTangramXmlParse(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CTangramHtmlTreeWnd::OnInsertData(WPARAM wParam, LPARAM lParam)
{
	...
	if(nPos!=-1)
	{
		...
		{
			map<CString,HTREEITEM>::iterator it = m_mapNodeDic.find(strID);
			if(it!=m_mapNodeDic.end())
			{
				CTangramXmlParse m_Parse;
