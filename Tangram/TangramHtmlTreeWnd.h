...
struct CTangramXHtmlTreeNode
{
	...
	virtual ~CTangramXHtmlTreeNode()
	{
		...
		m_pszNote = 0;
		m_nCount--;
		if(m_pTangramTreeNode)
		{
			DWORD dwRef = m_pTangramTreeNode->Release();
			while(dwRef)
			{
				dwRef = m_pTangramTreeNode->Release();
			}
		}
	}
	...
	ITangramTreeNode*	m_pTangramTreeNode;

	map<CString, CWndNode*> m_mapBindNode;
	HTREEITEM m_hItem;
...
public:
	...
	afx_msg LRESULT OnGetTangramXmlParse(WPARAM wParam, LPARAM lParam);
