...
JNIEXPORT void setInitialArgs(int argc, _TCHAR** argv, _TCHAR* lib) {
	...
	if (pDisp)
	{
		...
		_AtlModule.m_pJavaProxy = new CComObject<CTangramJavaProxy>;
		_AtlModule.m_pJavaProxy->AddRef();
		m_pTangram->put_JavaProxy(_AtlModule.m_pJavaProxy);
	}
}
...
static int _run(int argc, _TCHAR* argv[], _TCHAR* vmArgs[])
{
    ...
    if (launchMode == LAUNCH_JNI) {
		...
		//	
		//	CString strPath = CString(cp) + _T(";") + CString(szPath);
		//	int nPos = strPath.ReverseFind('.');
		//	strPath = strPath.Left(nPos);
		//	strPath += _T(".jar");
		//	cp = strPath.GetBuffer();
	...
	if (vmArgs != NULL) {
		for (int index = 0; vmArgs[index] != NULL; index++)
		{
			free(vmArgs[index]);
		}
	}
...
static void adjustVMArgs(_TCHAR *javaVM, _TCHAR *jniLib, _TCHAR **vmArgv[]) {
	...
	if (permGen != NULL) {
		...
		if (!specified && isMaxPermSizeVM(javaVM, jniLib)) {
			...
			_TCHAR *newArg = (_TCHAR *)malloc((_tcslen(XXPERMGEN) + _tcslen(permGen) + 1) * sizeof(_TCHAR));
			_stprintf(newArg, _T_ECLIPSE("%s%s"), XXPERMGEN, permGen);

			*vmArgv = (_TCHAR **)malloc((i + 2) * sizeof(_TCHAR *));
